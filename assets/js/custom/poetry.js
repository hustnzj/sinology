document.addEventListener("DOMContentLoaded", function () {
  ...
  pTags.forEach((pTag) => {
    ...
    if (lines.length >= 2) {
      ...
      // 检查第一行是否只包含拼音、空格和可选的《
      const isPinyinLine =
        /^[《]?[a-zāáǎàēéěèīíǐìōóǒòūúǔùǖǘǚǜ\s]*[》]?[a-zāáǎàēéěèīíǐìōóǒòūúǔùǖǘǚǜ\s]*$/.test(
          firstLine
        );
  ...
  function generateRubyHTML(lines) {
    ...
    //汉字行正则，用于提取汉字、中文标点、脚注sup、用来生成高亮波浪线的#号
    const chineseSegRegex = new RegExp(
      `(${hanziRegex.source})|(${chinesePunRegex.source})|(${supFootNoteRegex.source})|#`,
      "g"
    );
    ...
    for (let i = 0; i < lines.length; i += 2) {
      ...
      //获得拼音行数组
      const pinyinSegments = pinyinLine
        .replace(supFootNoteRegex, "")
        .replace(chinesePunRegex, " ")
        .split(" ")
        .filter(Boolean);

      //汉字行数组
      const chineseSegments = chineseLine
        .match(chineseSegRegex)
        .filter(Boolean);
      if (chineseLine.includes("##")) console.log(chineseSegments);

      let pinyinIndex = 0;
      let isWavy = false; // 标记是否进入了 ## 包裹的状态
      let hashCount = 0; // 记录 # 的数量
      ...
      for (let i = 0; i < chineseSegments.length; i++) {
        ...
        if (segment.startsWith("<sup")) {
          ...
          rubyHTML += segment;
        } else if (chinesePunStr.includes(segment)) {
          // 此方法直接检查字符串是否包含特定的子字符串，它在底层处理的是字符，而不是使用正则表达式。因此，它可以正确识别和匹配输入的字符，比如全角标点符号。这是因为它不涉及编码转换或复杂的匹配机制，只是逐字符比较。
